...
const showModal = (id) =>{
    ...
    currentCategory.value = props.categories.data.find((category) => category.id === id)
}
const closeModal = () =>{
...
const editCategory = (id) =>{
    if(currentCategory.value.name === props.categories.data.find(category => category.name === currentCategory.value.name && category.id !== id)?.name){
...
<template>
    <div class="bg-white p-4 rounded-2xl">
        <table class="w-full mt-2 border">
            ...
            <tbody>
                <tr v-for="value in categories.data" :key="value.id">
                    ...
                    <td class="border p-2 flex gap-2 justify-center">
                        ...
                        <button @click="deleteCategory(value.id)" class="bg-dangerRed hover:bg-red-700 text-white p-1 rounded"><img src="/assets/icons/svg/actions/delete.svg" alt="Eliminar" class="w-6"></button>
                    </td>
                </tr>
            </tbody>
        </table>
            ...
            <div class="p-6">
                ...
                <form class="flex flex-col">
                    <div>
                        ...
                        <input v-model="currentCategory.name" type="text" placeholder="Nombre de Categoría" class="border p-2 rounded-lg mb-4" />
                        <InputError :message="form.errors.name"></InputError>
                        <span class="text-red-500 hidden">Este nombre ya está en uso</span>
                    </div>

                    <div>
                        <InputLabel for="description" value="Descripción"></InputLabel>
                        <input v-model="currentCategory.description" type="text" placeholder="Descripción" class="border p-2 rounded-lg mb-4" />
                    ...
                    <div class="flex justify-between">
                        ...
                        <button @click="editCategory(currentCategory.id)" type="button" class="bg-approveGreen hover:bg-green-600 text-white px-4 py-2 rounded-lg self-end">Guardar Cambios</button>
                    </div>
