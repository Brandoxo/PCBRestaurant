...
class SalesController extends Controller
{
    public function index(Request $request) {
        ...
        $shifts = Shifts::all();
        $roomServiceConfig = ConfigRoomService::select('is_active', 'service_cost')->get();
    ...
    public function getSalesForCutOff(Request $request)
    {
        ...
        if ($shiftName) {
            ...
            if ($shift) {
                $query->whereTime('date_time', '>=', $shift->start_time)
                      ->whereTime('date_time', '<', $shift->end_time);
            }
        }

        return response()->json($query->orderBy('date_time', 'asc')->get());
    }
}
